...
async def send_result_email(candidate_name: str, candidate_email: str, job_title: str,
                             final_score: float, is_qualified: bool):
    if is_qualified:
        ...
        <div style="background:#D1FAE5;padding:20px;border-radius:8px;text-align:center;">
          ...
          <p style="font-size:24px;font-weight:bold;color:#047857;">Score: {final_score:.1f}%</p>
        ...
        <div style="background:#FEE2E2;padding:20px;border-radius:8px;text-align:center;">
          ...
          <p style="font-size:24px;font-weight:bold;color:#DC2626;">Score: {final_score:.1f}%</p>
          <p>Thank you for your time. We encourage you to apply for future opportunities.</p>
        </div>"""

    html = f"""
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
      <h2 style="color:#4F46E5;">SmartScreen — Application Result</h2>
      <p>Dear <b>{candidate_name}</b>,</p>
      <p>Here is the result of your application for <b>{job_title}</b>:</p>
      {status_html}
    </div>"""
